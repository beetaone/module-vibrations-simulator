...
PARAMS = {
    "SAMPLE_SIZE": int(
        getenv("SAMPLE_SIZE", 1024)
    ),  # number of samples taken per second, sample rate of 1024 means, 1024 values of the signal are recorded in one second
    "MEASUREMENT_DURATION": int(
        getenv("MEASUREMENT_DURATION", 5)
    ),  # how often the measurements are simulated, measurements interval
    ...
    "VIBRATION_PROBABILITY": float(
        getenv("VIBRATION_PROBABILITY", 0.1)
    ),  # probability of noise in the generated data
